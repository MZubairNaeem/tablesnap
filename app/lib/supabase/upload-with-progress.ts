 ...
 *
 * Takes the Supabase URL/key explicitly rather than reading them off
 * `process.env` -- in Nuxt those live in `useRuntimeConfig().public.supabase`
 * and the caller (a component, where the composable is available) is what
 * should read them.
 */
export function uploadWithProgress(
  ...
  credentials: { supabaseUrl: string; key: string }
): Promise<string> {
  const { supabaseUrl, key } = credentials;
  ...
  const contentType = blob.type.split(";")[0] || blob.type;
